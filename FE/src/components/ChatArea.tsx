...
export default function ChatArea({ chatMode, privateChatUserId, onBack }: Props) {
    ...
    }, [audioUrl, audioBlob, recordingSeconds, pendingMedia, input, chatMode, currentRoom, privateChatUserId, currentChatId, sendChat, sendRoomChat, sendPrivateChat, sendVoiceChat, sendRoomVoice, sendPrivateVoice, sendTypingStop, makeLocal, addLocalMessage, setPendingMedia]);
    ...
    }, [addLocalMessage, chatMode, currentChatId, currentRoom, makeLocal, privateChatUserId, sendChat, sendPrivateChat, sendRoomChat]);
    ...
    const openCamera = useCallback(() => {
        setShowAttachment(false);
        setShowCamera(true);
    }, [setShowAttachment, setShowCamera]);
    const openPoll = useCallback(() => {
        setShowAttachment(false);
        setShowPoll(true);
    }, [setShowAttachment, setShowPoll]);
    const ATTACHMENT_OPTIONS = useMemo(() => ([
        { id: 'photo', label: 'Photo or Video', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><circle cx="8.5" cy="8.5" r="1.5" /><polyline points="21 15 16 10 5 21" /></svg> },
        { id: 'file', label: 'File', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z" /><polyline points="13 2 13 9 20 9" /></svg> },
        { id: 'camera', label: 'Camera', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" /><circle cx="12" cy="13" r="4" /></svg> },
        { id: 'poll', label: 'Poll', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10" /><line x1="12" y1="20" x2="12" y2="4" /><line x1="6" y1="20" x2="6" y2="14" /></svg> },
        { id: 'location', label: 'Location', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg> },
    ]), []);
    const handleAttachmentOption = useCallback((id: string) => {
        if (id === 'photo') openPhotoPicker();
        else if (id === 'file') openFilePicker();
        else if (id === 'camera') openCamera();
        else if (id === 'poll') openPoll();
        else if (id === 'location') shareLocation();
                                ...
                                >
                                    {ATTACHMENT_OPTIONS.map((opt, i) => (
                                        <motion.button
                                            ...
                                            onClick={() => handleAttachmentOption(opt.id)}
